...
class Camera():
    ...
    def get_cameras(self): # get detected cameras 
        self.bsCam = pylon.InstantCamera(pylon.TlFactory.GetInstance().CreateFirstDevice())
        if not self.bsCam:
    ...
    def get_camera_writers(self): # open FFMPEG camera writers if we are saving to video
        if self.save_to_video:
            w, h = self.camera_config["acquisition"]["bsFrame_width"], self.camera_config["acquisition"]["bsFrame_height"]
            indict = self.camera_config["inputdict"].copy()
            indict['-s'] = '{}x{}'.format(w,h)
            self.bsCamWriter = skvideo.io.FFmpegWriter(self.video_files_names[0], outputdict = self.camera_config["outputdict"], inputdict = indict)
    ...
    def setup_cameras(self):
        # set up cameras
        print("Using camera: ", self.bsCam.GetDeviceInfo().GetModelName())
        self.bsCam.Open()
        ...
        self.bsCam.ExposureTime.FromString(self.camera_config["acquisition"]["bsExposure"])
        self.bsCam.Width.FromString(self.camera_config["acquisition"]["bsFrame_width"])
        self.bsCam.Height.FromString(self.camera_config["acquisition"]["bsFrame_height"])
        self.bsCam.Gain.FromString(self.camera_config["acquisition"]["bsGain"])
        self.bsCam.OffsetY.FromString(self.camera_config["acquisition"]["bsFrame_offset_y"])
            # if int(self.camera_config["acquisition"]["frame_offset_x"]) > 0:
        self.bsCam.OffsetX.FromString(self.camera_config["acquisition"]["bsFrame_offset_x"])
        ...
        if self.camera_config["trigger_mode"]:
            ...
            self.bsCam.MaxNumBuffer = 10 # Default is 10
        else:
